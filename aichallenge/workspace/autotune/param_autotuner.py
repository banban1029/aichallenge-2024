...
def objective(trial, param_ranges, param_files, target_param):
    ...
    p_autoware = subprocess.Popen("exec " + "bash /aichallenge/run_evaluation.bash", shell=True)
    # p_autoware = subprocess.Popen(["bash", "/aichallenge/run_evaluation.bash"], shell=False)

    # シミュレータを起動
    # p_awsim = subprocess.Popen(['/aichallenge/aichallenge_ws/AWSIM/AWSIM.x86_64'])
    # p_awsim = subprocess.Popen(['/aichallenge/simulator/AWSIM/AWSIM.x86_64'])
    ...
    # kill追加
    stop_process = subprocess.Popen(["bash", "/aichallenge/optuna-pkill_all.sh"], shell=False)

    sleep(10*2)

  #  subprocess.run("source /opt/ros/humble/setup.bash && source /aichallenge/workspace/install/setup.bash", shell=True)


    # update_ros2 = subprocess.Popen(["bash", "source /opt/ros/humble/setup.bash"], shell=False)
    # update_source = subprocess.Popen(["bash", "source /aichallenge/workspace/install/setup.bash"], shell=False)
...
def main():
    ...
    # Optunaでパラメータを最適化
    study = optuna.create_study(direction='minimize', study_name='Autoware_turning_study-2024-08-04', storage='sqlite:///Autoware_turning_study-2024-08-04.db', load_if_exists=True)
    study.optimize(lambda trial: objective(trial, param_ranges, param_files, target_param), n_trials=2)
